...
set hst_nm=`echo $rootname |awk -F. '{print $(NF-1)}'`
...
#   $nco_dir/ncclimo --no_stdin --model_name=$modelname --var_lst=$var_list --dec_md=$djf_md --case=$casename --yr_srt=$first_yr --yr_end=$yr_end --drc_in=$path_history --drc_out=$path_climo
   $nco_dir/ncclimo --no_stdin --clm_md=mth -m $modelname -v $var_list -a $djf_md -c $casename -s $first_yr -e $yr_end -i $path_history -o $path_climo --lnk_flg='Yes' --hst_nm=$hst_nm
else
   $nco_dir/ncclimo --no_stdin --clm_md=mth -m $modelname              -a $djf_md -c $casename -s $first_yr -e $yr_end -i $path_history -o $path_climo --lnk_flg='Yes' --hst_nm=$hst_nm
endif
...
if ( -e $path_climo/derived/${rootname}`printf "%04d" ${first_yr}`-01.nc ) then
   ...
   $nco_dir/ncclimo --no_stdin --clm_md=mth -m $modelname -v AODVIS    -a $djf_md -c $casename -s $first_yr -e $yr_end -i $path_climo/derived -o $path_climo/derived_climo --lnk_flg='Yes' --hst_nm=$hst_nm
