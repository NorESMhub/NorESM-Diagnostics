...
if ($regrid_1 == 1) then
  ...
  echo prefix_1_dir:$prefix_1_dir

  cd  ${prefix_1_dir}

  ls ${caseid_1}_???_????-????_climo.nc ${caseid_1}_???_????-????_ts.nc > files_to_regrid_1.out
  ...
  foreach file ($files)
    ...
    if ( -f $new_res_1"_"$file ) then
      echo "${new_res_1}_${file} exists, skipp regriding"
      continue
    else
...
if ($regrid_2 == 1) then
  ...
  ls ${caseid_2}_???_????-????_climo.nc ${caseid_2}_???_????-????_ts.nc > files_to_regrid_2.out
